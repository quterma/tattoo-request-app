...
export async function GET(request: NextRequest) {
  ...
  if (!code) {
    return NextResponse.redirect(`${origin}/${locale}/admin/forgot-password?error=reset`)
  }
  ...
  if (error) {
    logAuthFailure("reset_callback", error)
    return NextResponse.redirect(`${origin}/${locale}/admin/forgot-password?error=reset`)
  }

  return NextResponse.redirect(`${origin}/${locale}/admin/reset-password`)
}
