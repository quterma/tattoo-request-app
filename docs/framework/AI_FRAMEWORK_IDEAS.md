...
- 2026-07-25 — **`docs/files-structure.md`-outside-write-surface has now recurred a fourth time
  (Item 16, Round 1), and this occurrence brought a second, previously-unseen shape of the same
  problem.** Prior sightings: Item 5 (9 rounds, 3 of them re-litigating this exact finding — logged
  2026-07-24 above), Item 15 (same finding, resolved in one round — cited in this thread's Review 2/
  Response 2), and Item 17 (8 rounds, cited in the 2026-07-25 round-count entry above). Item 16's
  Round 1 hit it again in Review 2 (finding 1) — mandatory `pnpm structure` regenerated the file,
  outside the task's declared Allowed Write Surface — and this time the executor did the fix
  immediately (extend the surface, ask the owner, one round, no re-litigation), so the *known* half
  of the pattern is not the news. **The new half, surfaced in Review 3 (finding 3):** the task's own
  marker-supersession rule (every generated Item 16 replacement carries `__asset_TODO`, written this
  same round in response to Review 2 finding 2) implies a future edit to
  `src/shared/i18n/messages/en.json`'s `app.__meta_TODO` bookkeeping string — a **content** file, not
  a generated report — and that path wasn't in the write surface either. This was **not** foreseeable
  by STRAT at the task's original cut (2026-07-23): at that time the favicon was Claude-drawn and
  final, not a marker-carrying placeholder, so Scope §3's "every placeholder" rule did not yet reach
  it. It became foreseeable only inside this same session, the moment the mid-round pivot moved the
  favicon into the generated-placeholder category — at which point Scope §3's pre-existing rule
  started applying to it — but the executing session did not re-derive that consequence itself; Codex
  caught it two review rounds later. Two separate mid-task write-surface extensions were needed
  across three review rounds on what Round 1 itself describes as "assets + a marker sweep." — open
  for META to judge: (a) whether `docs/files-structure.md` should simply be treated as always-in-
  surface for every task (it is wholly generated by a mandatory command, never hand-authored, so
  declaring it per-task may be authorizing something that was never actually a real boundary); (b) an
  IMPL-session discipline note — when a mid-session pivot recategorizes an asset (here: favicon,
  "drawn, unmarked" → "generated, marker-carrying"), the same turn should re-derive every place the
  newly-applicable rule now reaches (marker-legend strings included, not only the obviously-touched
  referencing files), instead of leaving it for review to find one gap per round; (c) conclude both
  extensions were correctly small, correctly asked, correctly approved, and this is just what a
  marker-migrating, multi-round, owner-in-the-loop task costs — no doc change, only (a) if it recurs
  on a task that isn't marker-related. Full detail:
  `docs/project/reviews/REVIEW_2026-07-25_stage6-item16-round1-favicon-pivot-location-slots.md`
  (Review 2 finding 1, Review 3 finding 3, both Responses).
