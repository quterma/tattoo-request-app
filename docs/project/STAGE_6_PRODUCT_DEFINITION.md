...
**Operational assumption — Preparation/Aftercare distribution.** These pages are reached primarily via stable, shareable URLs manually sent by the artist at the right journey moment (the artist-sent link remains the primary path — it is timed to the client's journey). The product provides stable URLs and content; timing the artist-sent link is the artist's responsibility. **In-product fallback discovery is provided by links in the Process page's FAQ** (the answer to "How do I prepare — and what about healing?") so a client who has lost the artist's message can still reach the page; this FAQ answer is not primary navigation (PRD D9 keeps navigation to Home/Process/Request/Location) and does not surface the pages to a first-time Instagram visitor deciding fit — it sits at the end of Process, in a section a fit-deciding reader can skip. (Owner decision 2026-07-14 added footer links for this purpose, superseding the Stage 6 blueprint's original "no footer links / accepted risk" stance; owner decision 2026-07-23 moved the fallback from the footer to the Process FAQ — see PROJECT_DECISIONS.md, Preparation/Aftercare in-product discovery.)
