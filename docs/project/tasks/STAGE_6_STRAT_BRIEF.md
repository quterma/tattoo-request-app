...
| 6 — Process content | **task ready** (`STAGE_6_TASK_06_process_content.md`) — **the next thing to run.** Copy approved 2026-07-23; also carries the form intro, `INSTAGRAM_HANDLE` and `__meta_TODO` copy swaps. |
...
| 15 — Prep/Aftercare discovery → Process | **task ready** (`STAGE_6_TASK_15_prep_aftercare_discovery.md`). Removes the two footer links; **amends PRD §5 + FS §2 together**. **Must run AFTER Item 6** — it creates the replacement FAQ links; reversing the order leaves zero discovery. |
...
1. **Item 6 → Item 5 → Item 15**, strictly in that order (`ready`). Item 5's teasers deep-link into
   Item 6's Process sections, and Item 15 removes the footer links that Item 6's FAQ replaces —
   running 15 before 6 would leave Preparation/Aftercare with no in-product discovery at all. Each
   task carries its copy **verbatim and approved**: the executor implements it, it does not re-word
   it. Then **Item 16** (assets, `draft` — owner generates the images).
