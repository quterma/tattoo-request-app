...
- **Remaining execution order, as of 2026-07-26 (all other items are done):**
  1. `TOOLING_TASK_02` — the `pnpm shot` browser capability. First, because both remaining items
     need it and neither can verify itself without it.
  2. **Item 18** — visual consistency, in two checkpointed blocks (A: tokens + primitives;
     B: pages + form + hero). Block A's handoff must say plainly that the site is expected to look
     *transitional* at that checkpoint — its consumers still carry their old overrides.
  3. **Item 13** — cut the task file only after Item 18 lands, then run it. Its sweep must fail
     while any `__asset_TODO` remains, and Item 18 deliberately adds a tenth (the Home hero), so
     cutting it earlier would encode a marker count that is already wrong.
  Launch these one at a time. Item 18's two blocks touch the same files as each other and Item 13
  reads the whole public surface — running any two concurrently re-triggers the shared-index /
  shared-docs hazard this stage has already been bitten by twice.
